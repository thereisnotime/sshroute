...
func runShadowMode() {
	...
	passthrough := func() {
		if err := syscall.Exec(ssh.RealSSH, append([]string{ssh.RealSSH}, os.Args[1:]...), os.Environ()); err != nil { // #nosec G204 G702 -- RealSSH is a compile-time constant (/usr/bin/ssh); transparent passthrough is the intended behaviour
